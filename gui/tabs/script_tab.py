...
class ScriptTab(BaseTab):
    ...
    def _export_list(self) -> None:
        ...
        payload = {
            "version": 1,
            "files": [
                {"path": f, "required": self.script_weights.get(f, 0) > 0}
                for f in self.script_files
            ],
        }
        try:
            Path(out).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            self.app.log(f"✓ 已匯出 {len(self.script_files)} 筆素材清單 → {out}")
        except Exception as exc:
    ...
    def _import_list(self) -> None:
        ...
        for entry in data["files"]:
            ...
            if path not in self.script_files:
                self.script_files.append(path)
                added += 1
            self.script_weights[path] = max(self.script_weights.get(path, 0), required)
